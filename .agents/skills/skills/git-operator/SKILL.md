...
5. Gerar commit com Conventional Commits em português e identificador da história.
...
Gere a mensagem sempre em **PORTUGUÊS**.
Execute o comando:
...
Não encerre sua atuação com “pronto para commit” ou “pronto para tag”.

O fechamento correto é apenas:
...
5. **Force Push**:
    - Como o histórico foi reescrito localmente, é necessário forçar a atualização remota.
    - `git push origin HEAD --force-with-lease`
    - _Nota: `--force-with-lease` é preferível a `--force` pois evita sobrescrever trabalho de terceiros se a branch
      remota tiver mudado._
