...
- Publicar artifact Android de homologação na pipeline quando a URL HTTPS de homologação e a assinatura controlada estiverem configuradas.
- Criar contrato da pasta `artifacts/homologation` para versões estáveis de homologação.
- Registrar metadados mínimos de versão, commit, URL de backend, checksum, modo de dados e ponteiro para asset versionado.
- Versionar o APK Android estável como asset do GitHub Release da tag semântica, sem armazenar o binário dentro do git.
- Bloquear promoção de APK debug, assinatura debug, backend local/privado ou URL sem HTTPS.
- Exigir allowlist de host de homologação e validação real do certificado do APK por fingerprint SHA-256.
...
- A pipeline publica artifact Android de homologação quando `WORKLINK_HOMOLOGATION_API_BASE_URL` estiver configurada e os secrets de assinatura Android existirem.
