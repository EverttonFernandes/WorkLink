...
class DiscoveryFilterState {
  ...
  List<String> get availableCategoryNames {
    return availableProfessionals
        .map((professional) => professional.categoryName)
        .toSet()
        .toList()
      ..sort();
  }
  ...
  List<DiscoveryProfessional> get filteredProfessionals {
    return availableProfessionals
        .where(
          (professional) => professional.matchesCategory(selectedCategoryName),
        )
        .where(
  ...
  DiscoveryFilterState copyWith({
    ...
    return DiscoveryFilterState(
      ...
      selectedCityDisplayName: clearCity
          ? null
          : selectedCityDisplayName ?? this.selectedCityDisplayName,
