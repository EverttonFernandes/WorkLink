...
.PHONY: up down restart logs api db redis storage migrate clean backend-static-analysis mobile-static-analysis static-analysis backend-unit-test backend-integration-test backend-test backend-image-build mobile-static-analysis mobile-unit-test mobile-screen-test mobile-integration-test mobile-android-build mobile-test functional-test test-unit test-integration test-functional db-up db-down db-logs db-migrate test ci
...
mobile-unit-test: $(COMPOSE_ENV_FILE)
	$(DOCKER_COMPOSE) run --rm mobile-tests sh -lc "rm -rf coverage && flutter pub get && flutter test test/unit --coverage && awk -F: '/^LF:/{lf+=$$2}/^LH:/{lh+=$$2} END { if (lf == 0) { print \"No coverage data found\"; exit 1 } else { coverage=lh/lf*100; print \"Coverage: \" coverage \"%\" } }' coverage/lcov.info"

mobile-screen-test: $(COMPOSE_ENV_FILE)
	$(DOCKER_COMPOSE) run --rm mobile-tests sh -lc "rm -rf coverage && flutter pub get && if find test -type f \( -path '*/widget/*' -o -path '*/widgets/*' -o -path '*/screen/*' -o -path '*/screens/*' \) -name '*_test.dart'; then flutter test test/widget test/screen --coverage; else echo 'No screen/widget tests found'; fi"
